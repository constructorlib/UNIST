...
d3.csv("data/life_expectancy_by_country.csv")
  .then((data) => {
    // Fixed error where D3 was not defined, 1/5
    // console.log("object", data); is taking data correctly now
    // Declare variables
    var differnece_list = [];
    ...
    for (d of data) {
      if (current_country != d.country_name) {
        if (current_country_data.length > 1) {
          minmax = d3.extent(current_country_data, function (d) {
            return d.value;
          });
          var diff = {
    ...
    var differnece_list = differnece_list
      .sort((a, b) => a.difference - b.difference)
      .slice(0, 5);
    console.log("Data after sort: ", differnece_list);
    ...
    var processedData = data.filter((d) =>
      country_list.includes(d.country_name)
    );
