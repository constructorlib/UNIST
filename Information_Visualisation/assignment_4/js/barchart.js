class barChart {
  ...
  initData(data) {
    try {
      ...
      this.currentContinentList = [
        ...new Set(filteredData.map((item) => item.continent)),
      ];
  ...
  drawBarChart(data) {
    ...
    bar_enter
      ...
      .attr("y", function (d, i) {
        return yScale(locations[i]) + yScale.bandwidth() * 0.55;
      })
      .attr("font-size", 10)
      .style("fill", "#000000");
  ...
  filterBarDataBySelection(selectionList) {
    ...
    const currentData = bar.data.filter((d) => {
      return bar.currentContinentList.includes(d.continent);
    });
    var filteredData = currentData.filter((d) =>
      selectionList.includes(d.location)
    );
    if (filteredData.length == 0) filteredData = currentData;

    // Get latest datum of each country
    var processedData = [];
    var countryList = [];
    for (var d of filteredData) {
      if (!bar.currentContinentList.includes(d.continent)) continue;
      if (!countryList.includes(d.location)) {
        processedData.push(d);
        countryList.push(d.location);
      }
